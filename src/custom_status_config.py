...
def load_custom_status_config(config_path=None):
    ...
    if not os.path.exists(config_path):
        logger.debug(
            "Custom status config file not found at %s, skipping custom statuses",
            config_path,
        )
        return {}
    ...
    try:
        ...
        if not custom_statuses:
            logger.warning(
                "Custom status config file %s contains no custom_statuses", config_path
            )
            return {}
        ...
        for status in custom_statuses:
            ...
            if not field_name:
                logger.warning(
                    "Custom status entry missing 'field_name', skipping: %s", status
                )
                continue
            ...
            status_map[field_name] = {
                ...
                "metric_name": status.get(
                    "metric_name", field_name.replace("_count", "")
                ),
                "description": status.get(
                    "description",
                    f"Number of {status.get('metric_name', field_name)} tests",
                ),
            }
