...
```yaml
scrape_configs:
  - job_name: 'testrail-exporter'
    static_configs:
      - targets: ['localhost:9001']
        labels:
          instance: 'testrail-exporter'
```

If running in Docker, replace `localhost` with the appropriate hostname or IP address.
...
### Count of failed tests per run

```promql
test_run_failed_count
```

### Total passed tests across all runs

```promql
sum(test_run_passed_count)
```

### Failed test results with details
...
```bash
docker logs testrail-exporter
```

### Verify Environment Variables

```bash
docker exec testrail-exporter env | grep TESTRAIL
```
...
1. **Connection errors**: Verify `TESTRAIL_BASE_URL` is correct and includes the full path with `/api/v2/`
2. **Authentication errors**: Ensure `TESTRAIL_API_KEY` and `TESTRAIL_USERNAME` are correct
3. **No metrics**: Check that test runs exist for the specified `TESTRAIL_PROJECT_ID` within the `LOOKBACK_DAYS` period
4. **Port conflicts**: Change `METRICS_PORT` if port 9001 is already in use
